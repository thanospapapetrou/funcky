...
public class Parser {
        ...
        switch (token.type()) {
            ...
            case SYMBOL:
                if (peek(input, union(Set.of(TokenType.PERIOD, TokenType.SPACE), follow)).type()
                    ...
                    consume(input, TokenType.PERIOD);
                    return new FunckyReference(token.file(), token.line(), token.column(),
                            token.value(), consume(input, TokenType.SYMBOL).value());
                }
                return token.value().equals(TYPE_VARIABLE)
                        ? new FunckyLiteral(token.file(), token.line(), token.column(), new FunckyTypeVariable())
    ...
    private FunckyLiteral parseList(final List<FunckyExpression> elements, final Token leftSquareBracket) {
        ...
        final FunckyListType listType = (FunckyListType) new FunckyListType(new FunckyLiteral((head == null)
                ? new FunckyTypeVariable() : head.getType())).unify((tail == null)
                ? new FunckyListType(new FunckyLiteral(new FunckyTypeVariable())) : tail.getType());
            if (listType == null) {
        ...
        return new FunckyLiteral(leftSquareBracket.file(), leftSquareBracket.line(), leftSquareBracket.column(),
                new FunckyList(listType, head, tail));
    }
